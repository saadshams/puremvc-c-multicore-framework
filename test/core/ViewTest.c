...
void testGetInstance() {
    ...
    struct IView *view = puremvc_view_getInstance("ViewTestKey1", puremvc_view_new, &error);
    view->initializeView(view, &error);

    // test assertions
    assert(view != NULL);
    assert(view == puremvc_view_getInstance("ViewTestKey1", puremvc_view_new, &error));
...
void testRegisterAndNotifyObserver() {
    ...
    struct IView *view = puremvc_view_getInstance("ViewTestKey2", puremvc_view_new, &error);
    view->initializeView(view, &error);
    ...
    view->registerObserver(view, "ViewTestNote1", observer, &error);
    ...
    vo->value = 10;
    struct INotification *note = puremvc_notification_new("ViewTestNote1", vo, NULL, &error);
    view->notifyObservers(view, note, &error);
...
void testRegisterAndRetrieveMediator() {
    // Get the Multiton View instance
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey3", puremvc_view_new, &error);
    view->initializeView(view, &error);
    ...
    // clean up
    view->removeMediator(view, "testing", &error);
...
void testHasMediator() {
    // Get the Multiton View instance
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey4", puremvc_view_new, &error);
    view->initializeView(view, &error);
    ...
    view->removeMediator(view, "hasMediatorTest", &error);
...
void testRegisterAndRemoveMediator() {
    // Get the Multiton View instance
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey6", puremvc_view_new, &error);
    view->initializeView(view, &error);
    ...
    const struct IMediator *removedMediator = view->removeMediator(view, "testing", &error);
...
void testOnRegisterAndOnRemove() {
    // Get the Multiton View instance
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey5", puremvc_view_new, &error);
    view->initializeView(view, &error);
    struct ViewTest viewTest = {"", false, false, 0};

    // Create and register the test mediator
    struct IMediator *mediator = test_mediator4_new(&viewTest, &error);
    view->registerMediator(view, mediator, &error);
    ...
    // Remove the component
    view->removeMediator(view, ViewTestMediator4_NAME, &error);
...
void testSuccessiveRegisterAndRemoveMediator() {
    // Get the Multiton View instance
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey7", puremvc_view_new, &error);
    view->initializeView(view, &error);

    // Create and register the test mediator,
    // but not so we have a reference to it
    struct ViewComponent viewComponent = {};
    struct IMediator *mediator = test_mediator_new(&viewComponent, &error);
    ...
    // Remove the Mediator
    view->removeMediator(view, ViewTestMediator_NAME, &error);

    // test that retrieving it now returns null
    assert(view->removeMediator(view, ViewTestMediator_NAME, &error) == NULL);
...
void testRemoveMediatorAndSubsequentNotify() {
    // Get the Multiton View instance
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey8", puremvc_view_new, &error);
    view->initializeView(view, &error);

    // Create and register the test mediator to be removed.
    struct ViewTest viewTest = {.lastNotification = NULL };
    struct IMediator *mediator = test_mediator2_new(&viewTest, &error);
    view->registerMediator(view, mediator, &error);

    // test that notifications work
    struct INotification *notification1 = puremvc_notification_new(NOTE1, NULL, NULL, &error);
    view->notifyObservers(view, notification1, &error);
    assert(strcmp(viewTest.lastNotification, NOTE1) == 0);

    struct INotification *notification2 = puremvc_notification_new(NOTE2, NULL, NULL, &error);
    view->notifyObservers(view, notification2, &error);
    assert(strcmp(viewTest.lastNotification, NOTE2) == 0);

    view->removeMediator(view, ViewTestMediator2_NAME, &error);
...
void testRemoveOneOfTwoMediatorsAndSubsequentNotify() {
    // Get the Multiton View instance
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey9", puremvc_view_new, &error);
    view->initializeView(view, &error);
    struct ViewTest viewTest = {};

    // Create and register that responds to notifications 1 and 2
    struct IMediator *mediator2 = test_mediator2_new(&viewTest, &error);
    view->registerMediator(view, mediator2, &error);

    // Create and register that responds to notification 3
    struct IMediator *mediator3 = test_mediator3_new(&viewTest, &error);
    ...
    struct INotification *notification3 = puremvc_notification_new(NOTE3, NULL, NULL, &error);
    view->notifyObservers(view, notification3, &error);
    ...
    view->removeMediator(view, ViewTestMediator3_NAME, &error);
...
void testMediatorReregistration() {
    ...
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey10", puremvc_view_new, &error);
    view->initializeView(view, &error);

    // Create and register that responds to notification 5
    struct ViewTest viewTest = {};
    struct IMediator *mediator = test_mediator5_new(&viewTest, &error);
    ...
    // Remove the Mediator
    struct IMediator *removed = view->removeMediator(view, ViewTestMediator5_NAME, &error);
...
void testModifyObserverListDuringNotification() {
    // Get the Singleton View instance
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey11", puremvc_view_new, &error);
    view->initializeView(view, &error);
    ...
    // to change.
    view->registerMediator(view, test_mediator6_new("ViewTestMediator6/1", &viewTest, &error), &error);
    view->registerMediator(view, test_mediator6_new("ViewTestMediator6/2", &viewTest, &error), &error);
    view->registerMediator(view, test_mediator6_new("ViewTestMediator6/3", &viewTest, &error), &error);
    view->registerMediator(view, test_mediator6_new("ViewTestMediator6/4", &viewTest, &error), &error);
    view->registerMediator(view, test_mediator6_new("ViewTestMediator6/5", &viewTest, &error), &error);
    view->registerMediator(view, test_mediator6_new("ViewTestMediator6/6", &viewTest, &error), &error);
    view->registerMediator(view, test_mediator6_new("ViewTestMediator6/7", &viewTest, &error), &error);
    view->registerMediator(view, test_mediator6_new("ViewTestMediator6/8", &viewTest, &error), &error);
...
void testRemoveView() {
    ...
    const char *error = NULL;
    struct IView *view = puremvc_view_getInstance("ViewTestKey12", puremvc_view_new, &error);
    view->initializeView(view, &error);
